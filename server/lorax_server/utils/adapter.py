...
def load_module_map(
    ...
    for filename in adapter_filenames:
        if filename.suffix == ".safetensors":
            ...
            keyfile_filename = source.weight_files(".key")[0]
            with _read_stream(keyfile_filename) as f:
                encryption_key = f.read()
            with _read_stream(filename) as f:
                deserialized_adapter = TensorDeserializer(
                    f,
                    device="cpu",
                    dtype=torch.bfloat16,
                    encryption=DecryptionParams.from_key(encryption_key),
                )
                adapter_weights.update(deserialized_adapter)
