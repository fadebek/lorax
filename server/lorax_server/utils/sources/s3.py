...
def weight_s3_files(bucket: Any, model_id: str, extension: str = ".safetensors") -> List[str]:
    ...
    filenames = [
        ...
        if f.key.endswith(extension) or f.key.endswith(".tensors")
    ]
    if not filenames:
        raise EntryNotFoundError(
            f"No {extension} or .tensors weights found for model {model_id}",
            None,
        )
    return filenames
...
def weight_files_s3(
    ...
    if local_path.exists() and local_path.is_dir():
        local_files = list(local_path.glob(f"*{extension}")) + list(local_path.glob("*.tensors"))
        if not local_files:
            raise FileNotFoundError(
                f"No local weights found in {model_id} with extension {extension} or .tensors"
            )
        return local_files
...
def download_model_from_s3(bucket: Any, model_id: str, extension: str = ".safetensors"):
    ...
    filenames_with_extension = [
        f for f in model_files if f.key.endswith(extension) or f.key.endswith(".tensors")
    ]
